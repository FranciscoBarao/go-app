...
func TestFilterConditions(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			filters:  []listopt.Filter{{Column: "player_number", Operator: listopt.Eq, Value: "5", ValueKind: listopt.KindInt}},
			wantSQL:  " AND player_number = $1",
			wantArgs: []any{5},
		},
		{
			name:     "lt op with numeric value",
			filters:  []listopt.Filter{{Column: "player_number", Operator: listopt.Lt, Value: "5", ValueKind: listopt.KindInt}},
			wantSQL:  " AND player_number < $1",
			wantArgs: []any{5},
		},
		{
			...
			filters: []listopt.Filter{
				{Column: "min_players", Operator: listopt.Ge, Value: "3", ValueKind: listopt.KindInt},
...
func TestBuildPaginatedQuery(t *testing.T) {
	...
	q, args := buildPaginatedQuery("SELECT * FROM boardgames WHERE deleted_at IS NULL", query)
...
func TestBuildPaginatedQuery_DefaultOrderByIDWhenNoSort(t *testing.T) {
	query := listopt.NewQuery(listopt.WithPagination(listopt.Pagination{Page: 1, PageSize: 10}))
	q, _ := buildPaginatedQuery("SELECT * FROM boardgames WHERE deleted_at IS NULL", query)

	require.Contains(t, q, " ORDER BY id ASC")
}

func TestBuildPaginatedQuery_IDTiebreakWithSort(t *testing.T) {
	query := listopt.NewQuery(listopt.WithSort(listopt.Sort{Column: "name", Order: "asc"}), listopt.WithPagination(listopt.Pagination{Page: 1, PageSize: 10}))
	q, _ := buildPaginatedQuery("SELECT * FROM boardgames WHERE deleted_at IS NULL", query)
...
func TestBuildPaginatedQuery_SortByIDHasNoDuplicateTiebreak(t *testing.T) {
	query := listopt.NewQuery(listopt.WithSort(listopt.Sort{Column: "id", Order: "desc"}), listopt.WithPagination(listopt.Pagination{Page: 1, PageSize: 10}))
	q, _ := buildPaginatedQuery("SELECT * FROM boardgames WHERE deleted_at IS NULL", query)
...
func TestBuildCountQuery(t *testing.T) {
	query := listopt.NewQuery(listopt.WithFilter(listopt.Filter{Column: "name", Operator: listopt.Eq, Value: "Catan"}))
	q, args := buildCountQuery("SELECT COUNT(*) FROM boardgames WHERE deleted_at IS NULL", query)
