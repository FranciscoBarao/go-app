...
// DeleteBoardgameRatings removes all ratings for a boardgame.
const DeleteBoardgameRatings = `DELETE FROM boardgame_ratings WHERE boardgame_id = $1`
...
// SelectBoardgameRatings retrieves all ratings for a boardgame.
const SelectBoardgameRatings = `SELECT id, created_at, updated_at, username, value
